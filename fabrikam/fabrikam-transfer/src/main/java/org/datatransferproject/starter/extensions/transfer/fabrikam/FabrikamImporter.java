 ...
 *
 */
public class FabrikamImporter implements Importer<TokenAuthData, ContactsModelWrapper> {
    ...
    public ImportResult importItem(UUID jobId, TokenAuthData authData, ContactsModelWrapper wrapper) throws Exception {
        try (JCardReader reader = new JCardReader(wrapper.getVCards())) {
            ...
            for (VCard card : cards) {
                ...
                System.out.println(String.format("Fabrikam: Importing contact for %s %s", structuredName.getGiven(), structuredName.getFamily()));
            }
        }
