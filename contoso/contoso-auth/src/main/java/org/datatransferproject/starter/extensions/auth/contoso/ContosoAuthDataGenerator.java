...
/**
 * Generates authentication data for Contoso.
 *
 *
 * Since the demo does not authenticate against a live service, the OAuth url is set set directly to the callback address.
 */
public class ContosoAuthDataGenerator implements AuthDataGenerator {
    private static final AuthProtocol AUTH_PROTOCOL = OAUTH_2;

    @Override
    public AuthFlowConfiguration generateConfiguration(String callbackBaseUrl, String id) {
        // Normally the auth URL would be the URL of the OAuth provider. Since the demo Contoso extension does not use a real OAuth service, the configuration informs the UI
