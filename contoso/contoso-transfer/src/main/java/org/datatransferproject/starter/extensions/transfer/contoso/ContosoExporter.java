 ...
 *
 */
public class ContosoExporter implements Exporter<TokenAuthData, ContactsModelWrapper> {
    ...
    public ExportResult<ContactsModelWrapper> export(UUID jobId, TokenAuthData authData, Optional<ExportInformation> exportInformation) throws Exception {
        ...
        try (JCardWriter writer = new JCardWriter(stringWriter)) {
            ...
            writer.write(card);
            System.out.println(String.format("Contoso: Exporting contact for %s %s", structuredName.getGiven(), structuredName.getFamily()));
        }
